...
def test_recast_dialog_recasts_onto_the_cli_default_and_the_providers_own_effort(ui):
    """The picks that read as empty are choices too: choosing them must clear what the character carries,
    not be swallowed by it."""
    key = ui.store.stories.create("Back to default", "")
    chr_id = ui.store.stories.start(key, "", "claude-opus-5", "max")
    settled(ui, chr_id)
    open_story(ui, key)
    ui.click(ui.find(f"recastButton_{chr_id}"))
    assert ui.find("recastModel").property("editText") == "Opus 5"      # seeded from the character
    assert ui.find("recastEffort").property("currentText") == "max"
    ui.choose(ui.find("recastModel"), "cli default")
    ui.choose(ui.find("recastEffort"), "provider default")
    ui.click(ui.find("recastConfirm"))
    ch = ui.store.stories.character(chr_id)
    assert (ch["model"], ch["effort"]) == ("", "")
    cast = ui.store.contexts.get(ch["live_context"]).meta["cast"]
    assert (cast["model"], cast["effort"]) == ("", "")
