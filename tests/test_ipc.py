...
class FakeStories:
    ...
    def cast_call(self, character_id, note, as_name="", fork=False, model=None, effort=None, preset=None):
    ...
    def cast_create(self, character_id, title, description="", start=False, model=None, effort=None, preset=None):
...
def test_context_new_without_prompt_goes_through_contexts_create(h, store):
    ...
    assert store.contexts.calls == [("create", "bare", "bare one", {"model": None, "effort": None, "preset": None})]
    assert s["title"] == "bare one"


def test_context_new_without_prompt_or_title_defaults_title_to_new_context(h, store):
    h("context.new", {})
    assert store.contexts.calls == [("create", "bare", "New context", {"model": None, "effort": None, "preset": None})]
...
def test_a_story_verb_with_no_picks_in_it_passes_none_not_the_empty_pick(h, store):
    """An absent key is nobody picking. The empty string is a pick of its own — the cli default model,
    the provider's own effort — so the two cannot arrive at the store looking the same."""
    h("story.start", {"key": "ABC-1"})
    assert store.stories.calls[-1] == ("start", "ABC-1", "", None, None, None)
    h("story.start", {"key": "ABC-1", "model": "", "effort": ""})
    assert store.stories.calls[-1] == ("start", "ABC-1", "", "", "", None)
