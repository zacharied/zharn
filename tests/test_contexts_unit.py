...
def test_a_spawn_on_the_empty_effort_passes_no_effort_flag(store, tmp_path):
    """"" is a real entry in config.EFFORTS: the provider's own effort, which it gets by being told
    nothing. The position's "high" must not creep in under it."""
    cid = store.spawn("friend", "hello", effort="")
    assert wait_until(lambda: store.get(cid).status == "idle")
    argv = inits(tmp_path, cid)[0]["argv"]
    assert "--effort" not in argv
    assert store.get(cid).meta["cast"]["effort"] == ""


def test_a_spawn_with_no_effort_picked_carries_the_positions_own(store, tmp_path):
    cid = store.spawn("friend", "hello")
    assert wait_until(lambda: store.get(cid).status == "idle")
    argv = inits(tmp_path, cid)[0]["argv"]
    assert argv[argv.index("--effort") + 1] == cfg.CAST_POSITIONS["friend"]["effort"]
