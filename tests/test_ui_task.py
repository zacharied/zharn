...
def test_dispatch_button_spawns_thread_and_opens_its_tab(ui):
    ...
    assert ui.store.threads.model.count() == n + 1
    t = ui.store.threads.get(ui.store.threads.model.rows()[-1]["id"])
    assert t.taskKey == "ABC-3" and t.title == "do the thing via ui"
    assert ui.has(f"tab_thread_{t.id}")
...
def test_dispatch_with_unimplemented_provider_shows_error_banner(ui):
    ...
    ui.click(ui.find("dispatchButton"))
    assert ui.store.threads.model.count() == n, "no thread must be created"
