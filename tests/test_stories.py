...
class StubContexts(QObject):
    ...
    def create(self, position, **kw):
        if kw.get("model", "").startswith("gpt"):
...
class StubCasting:
    ...
    def resolve(self, position, model="", effort="", preset=""):
        ...
        return {"position": position, "label": pos["label"], "instructions": pos["instructions"],
                ...
                "model": model, "effort": effort or "high", "preset": preset or "full"}

    def rebuild(self, position, model="", effort="", preset=""):
...
def test_recast_of_a_working_character_waits_for_the_turn_boundary(store, contexts):
    ...
    assert store.character(chr_id)["recast_pending"] == {"model": "claude-opus-5", "effort": "", "preset": ""}
    settle(store, contexts, chr_id)
    ch = store.character(chr_id)
...
def test_max_line_pushes_then_recasts_at_the_turn_boundary_naming_the_cause(store, contexts):
    ...
    assert store.character(chr_id)["recast_pending"] == {"model": "", "effort": "", "preset": "", "cause": "context"}
...
def test_a_manual_recast_already_pending_keeps_its_picks_when_the_max_line_hits(store, contexts):
    ...
    assert store.character(chr_id)["recast_pending"] == {"model": "claude-sonnet-5", "effort": "", "preset": ""}
    settle(store, contexts, chr_id)
