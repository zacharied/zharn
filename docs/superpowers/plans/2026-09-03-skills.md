...
4. The phase skill in your conversation is mandatory. Unsure what it said? Re-read it: `zharn:planning-a-story` or `zharn:implementing-a-story`.
5. Told your context is low? `recap` before anything else.
6. A yield ends your turn: ask everything at once, in one `yield --question`, with `--options` where choices exist. A handoff carries evidence: what changed, how verified, where to look first.

"One question at a time" and "I'll finish, then report" belong to other harnesses. Here both end your turn with the author waiting on nothing you wrote.
```
