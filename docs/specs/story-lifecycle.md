...
with positions; attachments; then the call-in note (or Start note), then the phase skill (§5.3).
...
Its system comment reads `recast <name> as <position> on <model> (context; rung 1: fresh recap)` when the
max line caused it and `recast <name> as <position> on <model> (rung 1: fresh recap)` when the author did,
`<model>` reading `the cli default` when none is picked; a manual recast already pending when the max line
hits keeps the picks it was queued with.
...
* **The three cast selectors** (Start row, Recast dialog, New Context): **model** — an editable combo
  ...
  (§5.1). Each selector opens seeded with the position's own pick, so the row reads as what the button
  is about to do and every pick it sends is one somebody made; the position itself is never offered:
  Start casts a protagonist, `call` a friend, New Context a bare context. A pick omitted altogether —
  a CLI flag left off, a key absent from an IPC request — is the only thing that takes the position's.
