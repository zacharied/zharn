...
// Every pick may be left empty, and empty always means "the position's own".
RowLayout {
    id: picks
    ...
    readonly property var presets: [""].concat(app.casting.presetNames())
    // the empty entry is the one that has to explain itself: a picked "high" or "builder" reads on its own
    function anEffort(v) { return v || "default effort" }
    function aPreset(v) { return v || "default skills" }
    function labelOf(id) {
    ...
    Combo {
        ...
        model: picks.presets.map(picks.aPreset)
        ToolTip.visible: hovered && !popup.visible; ToolTip.delay: 700
