...
def render_brief(story: lc.Story, comments: list[dict], characters: dict[str, dict], note: str, *,
    ...
    threads. The position's instructions and the contract are in the system prompt (§5.3), not here."""
...
class StoryStore(QObject):
    ...
    def _on_usage(self, context_id: str):
        ...
        if tokens >= limit and not ch.get("context_maxed"):
            ...
            ch.setdefault("recast_pending", {"model": None, "effort": None, "preset": None, "cause": "context"})   # a manual one keeps its picks
    ...
    @intent
    def start(self, key, note="", model=None, effort=None, preset=None):
        return self._start(key, note, model, effort, preset)

    def _start(self, key, note="", model=None, effort=None, preset=None):
        key = self._key(key)
    ...
    @intent
    def reopen(self, key, note="", model=None, effort=None, preset=None):
        """Reopen resumes the protagonist (§2.1). With a model/effort/preset it recasts instead: the story
        reopens, the protagonist gets a fresh memory, and the note is delivered to that. A pick of "" is a
        pick — the cli default, the provider's own effort — so only "nothing at all" resumes."""
        if model is None and effort is None and preset is None:
    ...
    @Slot(str, str, str, str, str, result=str)
    @intent
    def recast(self, key, character_id, model=None, effort=None, preset=None):
    ...
    def _recast_now(self, ch: dict, model: str | None = None, effort: str | None = None,
                    preset: str | None = None, cause: str = "") -> str:
        """None is "this recast picked nothing", and the character keeps what it carries; "" is a pick in its
    ...
    def cast_create(self, character_id, title, description="", start=False, model=None, effort=None, preset=None) -> str:
    ...
    def cast_call(self, character_id, note, as_name="", fork=False, model=None, effort=None, preset=None) -> dict:
